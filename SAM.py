...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.sam1(x)  # Apply SAM after layer1
        x = self.pool(x)
        x = self.layer2(x)
        x = self.cam2(x)  # Apply CAM after layer2
        x = self.sam2(x)  # Apply SAM after layer2
        x = self.pool(x)
        x = self.layer3(x)
        x = self.cam3(x)  # Apply CAM after layer3
        x = self.sam3(x)  # Apply SAM after layer3
        x = self.pool(x)
        x = x.view(x.size(0), -1)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
